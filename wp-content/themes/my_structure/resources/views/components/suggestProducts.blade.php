   ...
   @if (!empty($relatedItems))
       <section class="mt-8" x-data>
                   ...
                   <ul class="grid gap-4 sm:gap-6 grid-cols-2 sm:grid-cols-3 lg:grid-cols-4" role="list">
                       @foreach ($relatedItems as $r)
                           @php
                               ...
                               // ---- base (prodotti singoli)
                               $stockBase = (int) ($r['disponibilita'] ?? ($r['stock'] ?? 0));
                               $available = (bool) ($r['available'] ?? $stockBase > 0);

                               // ---- KIT: disponibile solo se TUTTI i prodotti hanno disponibilita/stock > 0
                               if (!$isProduct) {
                                   $plist = [];
                                   if (!empty($r['products']) && is_array($r['products'])) {
                                       $plist = $r['products'];
                                   } elseif (!empty($kitsForJs[$rid]['products'])) {
                                       // fallback opzionale
                                       $plist = $kitsForJs[$rid]['products'];
                                   }

                                   $available = true;
                                   foreach ($plist as $p) {
                                       $pStock = is_array($p)
                                           ? (int) ($p['disponibilita'] ?? ($p['stock'] ?? 0))
                                           : (int) ($p->disponibilita ?? ($p->stock ?? 0));
                                       if ($pStock <= 0) {
                                           $available = false;
                                           break;
                                   ...
                                   // PER I KIT: non usare stockBase per il payload!
                                   $stockForUi = $available ? 1 : 0; // solo per la scritta "Qtà"
                                   $maxQty = $available ? 1 : 0; // per il carrello
                               } else {
                                   $stockForUi = $stockBase; // prodotti normali
                                   $maxQty = $stockBase;
                               }
                               ...
                               $payload = $r['cart'] ?? [
                                   ...
                                   'price' => (float) $price,
                                   'qty' => 1,
                                   'maxQty' => $maxQty, // <-- qui la correzione chiave
                               ];
                           @endphp
                           ...
                           <li>
                               <article
                                   ...
                                   <span
                                       class="text-[11px] mt-0.5 {{ $available ? 'text-green-300' : 'text-red-300' }}">
                                       {{ $available ? 'Disponibile' : 'Non disponibile' }}
                                       <span class="opacity-80">— Qtà: {{ $stockForUi }}</span>
                                       @if ($isProduct)
          ...
          {{ $available ? 'bg-[#45752c] text-white hover:bg-[#386322] focus:outline-none focus:ring-2 focus:ring-white/30' : 'bg-white/15 text-white cursor-not-allowed disabled:opacity-60' }}"
                                       @if ($available) @if ($isProduct)
            :disabled="isProductMaxed({{ $rid }}, {{ $stockForUi }})"
          @else
            :disabled="false" @endif
                                   @click.stop.prevent="addRecommendedToCart(@js($payload))" @else
                                       disabled @endif>
                                       Aggiungi
                                   </button>
                               </article>
