  <section x-data="shop()" id="shop" aria-labelledby="shop-title"
      ...
      <ul class="grid gap-6 grid-cols-2 min-[480px]:grid-cols-2 min-[768px]:grid-cols-3 min-[1024px]:grid-cols-4"
          role="list" aria-describedby="shop-title">
          @foreach ($products as $product)
                  ...
                  <article
                      ...
                      <div id="p-meta-{{ $product->id }}" class="flex items-center justify-between mt-2"
                          ...
                          <p class="text-[11px]" :class="{{ $remBind }} > 0 ? 'text-green-300' : 'text-red-300'">
                              ...
                              <link
                                  :href="'https://schema.org/' + ({{ $remBind }} > 0 ? 'InStock' : 'OutOfStock')"
                      ...
                      <div class="mt-2 grid gap-2 grid-cols-1 md:grid-cols-2">
                          <button type="button"
                              class="w-full text-xs font-semibold py-1.5 px-3 rounded transition
            ...
            addToCart(@js($productsForJs[$product->id]['cart'] ?? (object) []));
                          ...
                          <a href="{{ $product->url }}"
                              class="w-full text-center text-xs font-semibold py-1.5 px-3 rounded
