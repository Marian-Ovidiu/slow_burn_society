    ...
    @foreach ($latest as $kit)
      ...
      <li role="listitem" class="h-full">
        <article
          ...
          aria-labelledby="kit-ttl-{{ $kit->id }}">

          @if (!$available)
            <span class="absolute top-2 right-2 text-[10px] font-semibold px-2 py-0.5 rounded
                        bg-red-600 text-red    "
          ...
          <div class="text-left space-y-1 flex-1">
            <h3 class="text-sm font-semibold text-white truncate" id="kit-ttl-{{ $kit->id }}" itemprop="name">
              @if (!empty($kitUrl))
            ...
            <p class="text-sm text-white/90" itemprop="offers" itemscope itemtype="https://schema.org/Offer">
