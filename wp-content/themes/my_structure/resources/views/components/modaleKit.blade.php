<div x-show="modalOpen2" x-cloak x-transition class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 px-4"
    ...
    <div class="bg-white w-full max-w-md p-6 rounded relative max-h-[50vh] overflow-y-auto"
        ...
        <button class="absolute top-2 right-2 text-gray-500 hover:text-black"
            @click="modalOpen2 = false; selected2 = null" aria-label="Chiudi modale" type="button">✕</button>

        <template x-if="selected2">
            <div>
                <h2 id="modal2-title" class="text-xl font-bold mb-2" x-text="selected2.title ?? selected2.name"></h2>
                    ...
                    <div class="flex gap-2 mb-4 overflow-x-auto scrollbar-hide">
                        <template x-for="(img,i) in selected2.gallery" :key="i">
                            <img :src="img"
                                ...
                                @click="selectedImage2 = img" :alt="'Anteprima ' + (i + 1)" loading="lazy">
                        </template>
                    </div>
                </template>
                ...
                <button type="button"
                    ...
                    :disabled="!(Number(selected2?.disponibilita) > 0) || isInCart(selected2?.cart || {})"
...
<!-- LIGHTBOX (stesso scope) -->
<template x-if="selectedImage2">
    <div class="fixed inset-0 z-50 bg-black/80 flex items-center justify-center" @click="selectedImage2 = null"
        role="dialog" aria-modal="true">
        <img :src="selectedImage2" class="max-w-full max-h-[90vh] rounded shadow-xl" alt="Zoom immagine prodotto">
    </div>
</template>
