...
@section('content')
{{-- Pubblica la tua Stripe PK qui (o nel layout) --}}
<script>window.STRIPE_PK = window.STRIPE_PK || "{{ my_env('STRIPE_PK') ?? '' }}";</script>
...
>
  ...
  <div class="mt-2 h-1.5 w-full overflow-hidden rounded bg-gray-200">
    <div class="h-full bg-[#45752c] transition-all" :style="`width: ${progressWidth()}%`"></div>
  </div>
</div>

{{-- Alert se il carrello scade mentre sei qui --}}
<div
  x-show="expired"
...
>
  ...
  oppure <a href="/#shop" class="underline">torna allo shop</a> per ricrearlo.
</div>
...
<section x-data="checkout()" class="mx-auto w-full max-w-6xl px-4 py-6 md:py-10">
  ...
  <div x-show="$store.cartReady" class="grid grid-cols-1 gap-6 md:grid-cols-12 md:gap-8">
    ...
    <div class="md:col-span-7 space-y-6">
      {{-- Dati cliente (semplice, estendibile) --}}
      <div class="rounded-2xl border border-gray-200 bg-white p-4 md:p-6 shadow-sm">
        ...
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div>
            ...
            <input type="text" x-model="form.firstName" class="w-full rounded-lg border-gray-300 focus:border-black focus:ring-black" placeholder="Mario">
          </div>
          <div>
            <label class="mb-1 block text-sm text-gray-700">Cognome</label>
            <input type="text" x-model="form.lastName" class="w-full rounded-lg border-gray-300 focus:border-black focus:ring-black" placeholder="Rossi">
          </div>
          <div class="md:col-span-2">
            <label class="mb-1 block text-sm text-gray-700">Email</label>
            <input type="email" x-model="form.email" class="w-full rounded-lg border-gray-300 focus:border-black focus:ring-black" placeholder="mario@esempio.it">
          </div>
        </div>
      </div>
      ...
      <div class="rounded-2xl border border-gray-200 bg-white p-4 md:p-6 shadow-sm">
        ...
        <div class="mt-4 flex items-center justify-between text-xs text-gray-500">
          ...
          <div class="flex items-center gap-2">
            {{-- mini “badge” trust --}}
            <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M12 2l7 4v6c0 5-3.5 9-7 10-3.5-1-7-5-7-10V6l7-4z" stroke="currentColor"/></svg>
            <span>Stripe</span>
          </div>
        </div>
        ...
        <div class="mt-4 space-y-3">
          <button
            ...
            :disabled="loading || !$store.cart.items.length || !isFormValid()"
            @click="pay">
                  ...
                  <div class="min-w-0 flex-1">
                    <div class="flex items-start justify-between gap-2">
                      ...
                      <p class="text-sm font-semibold text-gray-900">
                        € <span x-text="$store.cart.lineSubtotal(it).toFixed(2).replace('.', ',')"></span>
                      </p>
                    </div>
                    <div class="mt-1 flex items-center justify-between text-xs text-gray-500">
                      <span>Qtà: <strong x-text="it.qty"></strong></span>
                      <span>Prezzo: € <span x-text="Number(it.price).toFixed(2).replace('.', ',')"></span></span>
                    </div>
                  </div>
                </div>
              </li>
