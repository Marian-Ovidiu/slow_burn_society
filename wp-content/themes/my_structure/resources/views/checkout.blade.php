                                            ...
                                            <div class="mt-2 flex items-center justify-between">
                                                ...
                                                <div class="text-xs text-gray-500" x-show="!editMode">
                                                    ...
                                                    <template x-if="$store.cart.maxFor(it.id) !== null">
                                                        <span class="ml-2">• Disp.: <span
                                                                x-text="$store.cart.maxFor(it.id)"></span></span>
                                                    </template>
                                                </div>
                                                ...
                                                <div class="flex items-center gap-2" x-show="editMode">
                                                    ...
                                                    <button type="button"
                                                        ...
                                                        :disabled="it.qty <= 1"
                                                        @click="$store.cart.setQty(it.id, it.qty - 1)">
                                                        −
                                                    </button>

                                                    <!-- input numerico -->
                                                    <input type="number"
                                                        class="w-14 h-8 rounded-lg border-gray-300 text-center text-sm focus:border-black focus:ring-black"
                                                        :value="it.qty" min="1"
                                                        :max="$store.cart.maxFor(it.id) ?? undefined"
                                                        @change="
                   $store.cart.setQty(it.id, Number($event.target.value || 1));
                   $event.target.value = $store.cart.qtyFor(it.id);
                 ">

                                                    <!-- + -->
                                                    <button type="button"
                                                        class="h-8 w-8 rounded-lg ring-1 ring-gray-300 text-gray-700 hover:bg-gray-50 disabled:opacity-40"
                                                        :disabled="($store.cart.maxFor(it.id) !== null) && (it.qty >= $store.cart
                                                            .maxFor(it.id))"
                                                        @click="$store.cart.setQty(it.id, it.qty + 1)">
                                                        +
                                                    </button>
                                                ...
                                                <div class="text-[11px] text-gray-500">
                                                    Prezzo: € <span
                                                        x-text="Number(it.price).toFixed(2).replace('.', ',')"></span>
                                                </div>
                                            </div>
